...
const ADMIN_ROLES = new Set(['admin', 'moderator', 'founder', 'co_founder']);
...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clear any stale admin session before setting the new one
      clearAdminAuth();
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">A5X Community · Restricted Access</p>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-border bg-secondary/30 p-3 text-center">
          <div className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span>
              Requires a user account with{' '}
              <span className="font-mono font-bold text-primary">admin</span> or{' '}
              <span className="font-mono font-bold text-primary">moderator</span> role
            </span>
          </div>
          <p className="text-[10px] text-muted-foreground mt-1">
            Set role in MongoDB Atlas → users collection → role field
          </p>
        </div>
