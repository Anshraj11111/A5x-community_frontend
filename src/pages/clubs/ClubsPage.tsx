...
export default function ClubsPage() {
  ...
  const handleJoin = async (e: React.MouseEvent, slug: string) => {
    ...
    try {
      ...
      error((err as { response?: { data?: { error?: { message?: string } } } })?.response?.data?.error?.message || 'Failed to join');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      {isLoading ? <InlineLoader /> : data?.data.length === 0 ? (
        <EmptyState icon={Users} title="No clubs found" description="Be the first to create a product club"
          action={isAuthenticated ? <Button size="sm" asChild><Link to="/clubs/new">Create a Club</Link></Button> : undefined} />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data?.data.map((club: IProductClub) => (
                ...
                <div className="relative h-24 bg-gradient-to-br from-secondary to-background overflow-hidden">
                  ...
                  {club.isPrivate && (
                    <div className="absolute top-2 right-2">
                      <Badge variant="secondary" className="text-[10px] gap-1"><Lock className="h-2.5 w-2.5" /> Private</Badge>
                    </div>
                  )}
                </div>
                <CardContent className="p-4 flex-1 flex flex-col">
                  <h3 className="font-semibold text-sm">{club.name}</h3>
                  <p className="text-xs text-muted-foreground mt-1 line-clamp-2 flex-1">{club.description}</p>
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      <Users className="h-3 w-3" /> {formatNumber(club.memberCount)} members
                    </span>
                    {!club.isMember ? (
                      <Button size="sm" variant="outline" className="h-7 text-xs px-3" onClick={(e) => handleJoin(e, club.slug)}>Join</Button>
                    ) : (
