...
import { ArrowUp, MessageSquare, Eye, Pin, Lock, Repeat2 } from 'lucide-react';
        ...
        <div className="flex gap-4">
          ...
          <div className="flex flex-col items-center gap-1 shrink-0">
            <Button
              variant="ghost"
              size="icon"
              className={cn('h-8 w-8 rounded-lg', post.voteScore > 0 ? 'text-primary' : 'text-muted-foreground')}
              onClick={handleUpvote}
              aria-label="Upvote"
            >
              <ArrowUp className="h-4 w-4" />
            </Button>
            <span className={cn('text-sm font-semibold tabular-nums', post.voteScore > 0 ? 'text-primary' : 'text-muted-foreground')}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <Badge variant={typeColors[post.type]} className="text-[10px]">{post.type}</Badge>
                ...
                {isAuthenticated && (
                  <button
                    ...
                    className={cn(
                      ...
                      hasReposted ? 'text-[#00FF88]' : 'hover:text-[#00FF88]'
                    )}
                    title={hasReposted ? 'Undo repost' : 'Repost'}
                  >
                    <Repeat2 className="h-3.5 w-3.5" />
                    {(post.repostCount || 0) > 0 && formatNumber(post.repostCount || 0)}
                  </button>
                )}
                <span className="flex items-center gap-1 text-xs">
